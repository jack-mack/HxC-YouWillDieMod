...
public class WorldData extends WorldSavedData {
	...
	public static String key = "WorldData";
    ...
    public static WorldData forWorld(World world) {
		...
		if(result == null) {result = new WorldData(); storage.setData(key, result); result.checkWorldData();}
	...
	private void checkWorldData() {
		if(potionValues == null) {
			...
			for(int i = 0; i < 12; i++) {
				int i2 = YouWillDie.YouWillDie.r.nextInt(Potion.potionTypes.length);

				boolean stop = false;
				while(Potion.potionTypes[i2] == null || !stop) {
					stop = true;
					i2 = YouWillDie.YouWillDie.r.nextInt(Potion.potionTypes.length);
			...
			for(int i = 0; i < 12; i++) {
				if(Potion.potionTypes[potionValues[i]] == null) {
					int i2 = YouWillDie.YouWillDie.r.nextInt(Potion.potionTypes.length);
					while(Potion.potionTypes[i2] == null) {i2 = YouWillDie.YouWillDie.r.nextInt(Potion.potionTypes.length);}
					potionValues[i] = i2;
				}
			}
		}
		...
		for(int i = 0; i < 12; i++) {if(potionDurations[i] != 0) {continue;} potionDurations[i] = 5 + YouWillDie.YouWillDie.r.nextInt(26);}

		if(mushroomColors == null) {
			mushroomColors = new int[13][2];

			for(int i = 0; i < 13; i++) {
				mushroomColors[i][0] = Color.HSBtoRGB(YouWillDie.YouWillDie.r.nextFloat(), YouWillDie.YouWillDie.r.nextFloat(), YouWillDie.YouWillDie.r.nextFloat());
				mushroomColors[i][1] = Color.HSBtoRGB(YouWillDie.YouWillDie.r.nextFloat(), YouWillDie.YouWillDie.r.nextFloat(), YouWillDie.YouWillDie.r.nextFloat());
			}
		}
		...
		if(changeDisadvantage) {
			currentDisadvantage = validDisadvantages[YouWillDie.YouWillDie.r.nextInt(validDisadvantages.length)];

			MinecraftServer server = FMLCommonHandler.instance().getMinecraftServerInstance();
			while(server != null && server.isHardcore() && currentDisadvantage.equals("Permadeath")) {currentDisadvantage = validDisadvantages[YouWillDie.YouWillDie.r.nextInt(validDisadvantages.length)];}
		}
		...
		if(currentTask == null) {
			...
			if(changeTask || (currentTask.equals("Kill") && currentTaskID == 0 && enderDragonKilled)) {giveNewTask();} else {if(currentTask.equals("Kill")) {currentTaskID %= validMobs.length;}}
		}

		if(rewardLevels == -1) {rewardLevels = 5 + YouWillDie.YouWillDie.r.nextInt(6);}
	}

	public void giveNewTask() {
		progress = 0;

		currentTask = validTasks[YouWillDie.YouWillDie.r.nextInt(validTasks.length)];

		if(currentTask.equals("Kill")) {
			currentTaskID = !enderDragonKilled ? YouWillDie.YouWillDie.r.nextInt(validMobs.length) : 1 + YouWillDie.YouWillDie.r.nextInt(validMobs.length - 1);
			currentTaskAmount = mobNumbers[currentTaskID][0] + YouWillDie.YouWillDie.r.nextInt(mobNumbers[currentTaskID][1]);
		} else if(currentTask.equals("Burn")) {
			currentTaskID = YouWillDie.YouWillDie.r.nextInt(validItems.length);

			if(validItems[currentTaskID].getItem() == Items.nether_star) {currentTaskAmount = 1;}
			else {currentTaskAmount = 5 + YouWillDie.YouWillDie.r.nextInt(28);}

			if(validItems[currentTaskID].getItem() instanceof ItemBlock) {currentTaskAmount /= 4;}
		}

		rewardLevels = 5 + YouWillDie.YouWillDie.r.nextInt(6);

		markDirty();
	}

	public String getDisadvantage() {
		return Config.Disadvantages ? currentDisadvantage : "None";
	}
}
