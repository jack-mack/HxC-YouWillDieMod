	...
	protected void onImpact(MovingObjectPosition par1MovingObjectPosition)
	{
		if (!worldObj.isRemote) {
			...
			if(list1 != null && !list1.isEmpty()) {
				...
				int j = type >= 12 ? (5 + YouWillDie.r.nextInt(26)) : getDataWatcher().getWatchableObjectInt(25);

				int damage = 0;

				if(type >= 12 || (getThrower().getEntityData().hasKey("Blessing") && getThrower().getEntityData().getString("Blessing").equals("Alchemist"))) {
					damage = YouWillDie.r.nextInt(Potion.potionTypes.length);
					while(Potion.potionTypes[damage] == null) {damage = YouWillDie.r.nextInt(Potion.potionTypes.length);}
				} else {
