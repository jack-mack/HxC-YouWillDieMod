...
public class ItemMysteryPotion extends Item {
	...
	public void registerIcons(IIconRegister par1IconRegister) {
		...
		for(int i = 0; i < 13; i++) {
			icons[i] = par1IconRegister.registerIcon("YouWillDie:mysteryPotion_" + (i + 1));
			icons[i + 13] = par1IconRegister.registerIcon("YouWillDie:mysteryPotionThrowable_" + (i + 1));
		}
		...
		if(par1ItemStack.getItemDamage() < 13) {
			...
			if(!par2World.isRemote) {
				...
				if(par1ItemStack.getItemDamage() % 13 >= 12) {
					value = YouWillDie.r.nextInt(Potion.potionTypes.length);
					while(Potion.potionTypes[value] == null) {value = YouWillDie.r.nextInt(Potion.potionTypes.length);}
				} else {
	...
	public ItemStack onEaten(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
		...
		String blessing = (par3EntityPlayer.getEntityData().hasKey("Blessing")) ? par3EntityPlayer.getEntityData().getString("Blessing") : null;
		...
		if((blessing == null || !blessing.equals("Alchemist")) && damage < 12) {
			...
			int value = YouWillDie.r.nextInt(Potion.potionTypes.length);
			while(Potion.potionTypes[value] == null) {value = YouWillDie.r.nextInt(Potion.potionTypes.length);}
			int time = 5 + YouWillDie.r.nextInt(26);
