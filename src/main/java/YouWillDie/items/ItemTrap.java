...
import YouWillDie.misc.BehaviorDispenseTrap;
...
public class ItemTrap extends Item {
	...
	public IIcon[] icons;
	public static String[] iconLocations = new String[] {"YouWillDie:itemTrap", "YouWillDie:trap2", "YouWillDie:trap3"};

	public ItemTrap() {
		super();
		setHasSubtypes(true);
		BlockDispenser.dispenseBehaviorRegistry.putObject(this, dispenseTrap);
        setUnlocalizedName("itemTrap");
	...
	public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10) {
		...
		if(par1ItemStack.stackSize == 0) {
			...
			return false;
		} else if(par5 == 255 && ModRegistry.blockTrap.getMaterial().isSolid()) {
			return false;
		} else if(par3World.canPlaceEntityOnSide(ModRegistry.blockTrap, par4, par5, par6, false, par7, par2EntityPlayer, par1ItemStack)) {
			Block block = ModRegistry.blockTrap;
			int j1 = getMetadata(par1ItemStack.getItemDamage());
			int k1 = ModRegistry.blockTrap.onBlockPlaced(par3World, par4, par5, par6, par7, par8, par9, par10, j1);
	...
	public boolean canPlaceItemBlockOnSide(World par1World, int par2, int par3, int par4, int par5, EntityPlayer par6EntityPlayer, ItemStack par7ItemStack) {
		...
		return par1World.canPlaceEntityOnSide(ModRegistry.blockTrap, par2, par3, par4, false, par5, null, par7ItemStack);
	}

	public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int metadata) {
		if(!world.setBlock(x, y, z, ModRegistry.blockTrap, metadata, 3)) {return false;}

		if(world.getBlock(x, y, z) == ModRegistry.blockTrap) {
            ModRegistry.blockTrap.onBlockPlacedBy(world, x, y, z, player, stack);
            ModRegistry.blockTrap.onPostBlockPlaced(world, x, y, z, metadata);
		}
