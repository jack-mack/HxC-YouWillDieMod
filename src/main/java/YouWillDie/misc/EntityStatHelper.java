...
public class EntityStatHelper {
	...
	public void entityJoinWorld(EntityJoinWorldEvent event) {
		if(!event.world.isRemote) {
			processEntity(event.entity, YouWillDie.r);

			boolean isClone;

			isClone = event.entity.getEntityData().hasKey("isClone") && event.entity.getEntityData().getBoolean("isClone");

			if(CommonTickHandler.worldData != null && CommonTickHandler.worldData.getDisadvantage().equals("Increased Mob Spawn") && (event.entity instanceof EntityMob) && !isClone && YouWillDie.r.nextInt(3) == 0) {
	...
	public void livingDeath(LivingDeathEvent event) {
		if(!event.entity.worldObj.isRemote) {
			if(event.entity.worldObj.getGameRules().getGameRuleBooleanValue("doMobLoot")) {
				...
				if(event.entity instanceof EntityLivingBase && event.source != null && event.source.getEntity() != null) {
					if(event.source.getEntity().getEntityData().hasKey("Blessing")) {
						...
						if(blessing.equals("Thief") && YouWillDie.r.nextInt(20) == 0) {
							if(!event.entity.worldObj.isRemote) {
				...
				if(YouWillDie.r.nextInt(30) == 0 || level == 5) {event.entity.entityDropItem(new ItemStack(ModRegistry.mysteryPotion, 1, YouWillDie.r.nextInt(13)), event.entity.height/2);}
			...
			if(CommonTickHandler.worldData.currentTask.equals("Kill") && WorldData.validMobs[CommonTickHandler.worldData.currentTaskID].isAssignableFrom(event.entity.getClass())) {
		...
		if(event.entity instanceof EntityPlayer) {
			...
			player.triggerAchievement(ModRegistry.losingIsFun);
			...
			if(Config.enableMobKillStats) {
				...
				if(count >= 5) {player.triggerAchievement(ModRegistry.theHunt);}
			}
			...
			if(Config.enableWeaponKillStats) {
				...
				if(count >= 10) {player.triggerAchievement(ModRegistry.jackOfAllTrades);}
			}
