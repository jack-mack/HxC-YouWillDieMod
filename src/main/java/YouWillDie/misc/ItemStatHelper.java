...
public class ItemStatHelper {
		...
		if(!event.entity.worldObj.isRemote) {
			...
			if(event.entity.getEntityData().hasKey("Blessing")) {
				...
				if(blessing.equals("Guardian")) {
					...
					event.source.getEntity().attackEntityFrom(damage, event.ammount * 0.07F);
				}
			}
			...
			if(!skip && event.source != null && event.source.getEntity() != null) {
				...
				if(Config.enableMobKillStats && event.source.getEntity() instanceof EntityPlayer && event.source.getEntity().getEntityData().hasKey("KillStats") && event.source.getEntity().getEntityData().getCompoundTag("KillStats").hasKey(mob)) {
				...
				String weapon = "misc";
				if(Config.enableWeaponKillStats && event.source.getEntity() instanceof EntityPlayer && event.source.getEntity().getEntityData().hasKey("WeaponStats") && event.source.getEntity().getEntityData().getCompoundTag("WeaponStats").hasKey(weapon)) {
	...
	public void onCrafting(ItemCraftedEvent event) {
		...
		if(!player.worldObj.isRemote) {
			...
			if(Config.enableCraftingStats && itemName != null) {
