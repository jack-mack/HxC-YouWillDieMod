package YouWillDie.tileentities;
...
public class TileEntityPillar extends TileEntity {
	...
	public void updateEntity() {
		...
		if(worldObj.isRemote) {
			...
			if(Config.pillarGlow) {worldObj.updateLightByType(EnumSkyBlock.Block, xCoord, yCoord, zCoord);}
		}
	}
	...
	public void spawnParticles() {
		...
		if(player != null && player.getEntityData().hasKey("Blessing") && player.getEntityData().getString("Blessing").equals(blessing)) {
			for(int i = 0; i < 2; i++) {worldObj.spawnParticle("portal", xCoord + YouWillDie.r.nextDouble(), yCoord + YouWillDie.r.nextDouble() * 2, zCoord + YouWillDie.r.nextDouble(), (YouWillDie.r.nextDouble() - 0.5D) * 2.0D, -YouWillDie.r.nextDouble(), (YouWillDie.r.nextDouble() - 0.5D) * 2.0D);}
		}
	}
	...
	public void writeToNBT(NBTTagCompound par1NBTTagCompound) {
		...
		if(blessing == null) {blessing = validBlessings[YouWillDie.r.nextInt(validBlessings.length)];}
	...
	public void readFromNBT(NBTTagCompound par1NBTTagCompound) {
		...
		blessing = par1NBTTagCompound.hasKey("Blessing") ? par1NBTTagCompound.getString("Blessing") : validBlessings[YouWillDie.r.nextInt(validBlessings.length)];
	}
